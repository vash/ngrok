...
type Configuration struct {
	...
	AuthToken          map[string]string               `yaml:"auth_token,omitempty"`
...
const (
	defaultConfigName = "ngrok.yaml"
	authConfigName    = "auth.yaml"
)

func LoadConfiguration(opts *Options) (config *Configuration, err error) {
	configPath := opts.config
	if configPath == "" {
		configPath = defaultPath(defaultConfigName)
	}
	authConfigPath := defaultPath(authConfigName)
	...
	authFile, err := os.OpenFile(authConfigPath, os.O_RDWR|os.O_CREATE, 0600)
	if err != nil {
		err = fmt.Errorf("Failed to open auth file %s: %v", authConfigPath, err)
		return
	}
	authBuf, err := io.ReadAll(authFile)
	if err != nil {
		err = fmt.Errorf("Failed to read auth file %s: %v", authConfigPath, err)
		return
	}
	...
	if err = yaml.Unmarshal(authBuf, &authMap); err != nil {
		err = fmt.Errorf("Error parsing configuration file %s: %v", configPath, err)
		return
	}
	config.AuthToken = authMap.AuthToken

	//TODO: Multi-host drops backwards compatibility since we do not save
	//this data
	...
	case "set-auth":
		if len(opts.args) < 2 {
			log.Error("You must specify both server and auth key e.g.: ngrok set-auth ngrok.me:4443 1234abcde")
			return
		}

		hostAddr, err := normalizeAddress(opts.args[0], "hostAddr")
		if err != nil {
			log.Error("Invalid address", err)
		}
		authConfigFile := defaultPath(authConfigName)
		SaveAuthToken(authConfigFile, hostAddr, opts.args[1])
		fmt.Printf("Authentication for host %s was set successfully in %s\n", hostAddr, authConfigFile)
...
func defaultPath(fileName string) string {
	...
	return path.Join(homeDir, ".config/ngrok/", fileName)
}
...
func SaveAuthToken(configPath, serverAddr, authtoken string) (err error) {
	...
	if c.AuthToken == nil {
		c.AuthToken = make(map[string]string)
	} else {
		if c.AuthToken[serverAddr] == authtoken {
			return
		}
	}

	// update auth token
	c.AuthToken[serverAddr] = authtoken
